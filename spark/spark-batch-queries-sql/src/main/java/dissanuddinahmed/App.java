  ...
  private static final String APP_NAME = "[SABD2324] Project 1 sql";
    ...
    public static void firtsQuerySql(SparkSession spark) {
      ...
      Dataset<Row> resDf = spark.sql(
        "SELECT date as date_of_failure, vault_id, failures_count " +
        "FROM (SELECT date, vault_id, COUNT(failure) as failures_count "+
        "FROM diskFailures "+
        "WHERE failure = '1' "+
        "GROUP BY date, vault_id "+
        "HAVING COUNT(failure) BETWEEN 2 AND 4) "+
        "ORDER BY failures_count"
      );
      //resDf.printSchema();
      //res.show();
    ...
    public static void secondQuerySql(SparkSession spark) {
      ...
      Dataset<Row> resModelDf = spark.sql(
        "SELECT model, COUNT(failure) as failures_count "+
        "FROM diskFailures "+
        "WHERE failure = '1' "+
        "GROUP BY model "+
      ...
      Dataset<Row> resultVaultPerModelDf = spark.sql(
        ...
        "  SELECT vault_id, COUNT(*) AS failures_count " +
        "  FROM diskFailures " +
        "  WHERE failure = 1 " +
        "  GROUP BY vault_id " +
        "), " +
        "topPerFailureVault AS ( " +
        "  SELECT vault_id, failures_count " +
        "  FROM perVaultFailure " +
        "  ORDER BY failures_count DESC" +
        "  LIMIT 10 " +
        "), " +
        "topPerVaultWithFailureDf AS ( " +
        "  SELECT tpf.vault_id, df.model, tpf.failures_count " +
        ...
        "), " +
        "vaultModelDf AS ( " +
        "  SELECT vault_id, failures_count, collect_set(model) AS list_of_models " +
        "  FROM topPerVaultWithFailureDf " +
        "  GROUP BY vault_id, failures_count " +
        "  ORDER BY failures_count DESC " +
        "  LIMIT 10 " +
        ") " +
        "SELECT vault_id, failures_count, concat_ws(';', models) AS models " +
        "FROM vaultModelDf"
